...
resource "azurerm_route_table" "has_demo_rt_spoke1" {
  name                = "has_demo_rt_spoke1"
  location            = azurerm_resource_group.has_demo_rg.location
  resource_group_name = azurerm_resource_group.has_demo_rg.name
}

resource "azurerm_route" "has_demo_route_to_spoke2" {
  name                = "has_demo_route_to_spoke2"
  resource_group_name = azurerm_resource_group.has_demo_rg.name
  route_table_name    = azurerm_route_table.has_demo_rt_spoke1.name
  address_prefix      = "10.2.0.0/16"
  next_hop_type       = "VirtualAppliance"
  next_hop_in_ip_address = "10.0.0.4"
}

resource "azurerm_subnet_route_table_association" "has_demo_rt_ass_spoke1_green" {
  subnet_id      = azurerm_subnet.has_demo_spoke1_subnet_green.id
  route_table_id = azurerm_route_table.has_demo_rt_spoke1.id
}

resource "azurerm_subnet_route_table_association" "has_demo_rt_ass_spoke1_red" {
...
resource "azurerm_virtual_network_peering" "has_demo_peer_hub_spoke1" {
  name                      = "has_demo_peer_hub_spoke1"
  resource_group_name       = azurerm_resource_group.has_demo_rg.name
  virtual_network_name      = azurerm_virtual_network.has_demo_vnet_hub.name
  remote_virtual_network_id = azurerm_virtual_network.has_demo_vnet_spoke1.id
  allow_virtual_network_access = true
  allow_forwarded_traffic      = true
  allow_gateway_transit = true
}

resource "azurerm_virtual_network_peering" "has_demo_peer_spoke1_hub" {
  name                      = "has_demo_peer_spoke1_hub"
  resource_group_name       = azurerm_resource_group.has_demo_rg.name
  virtual_network_name      = azurerm_virtual_network.has_demo_vnet_spoke1.name
